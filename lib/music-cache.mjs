...
// Contabiliza una reproducción SIN tocar disco ni object-store (modo worker: el
// audio vive en el disco del worker, Santiago solo lleva play_count/historial).
...
// Persiste metadata obtenida (título/duración) en la fila de la canción. Solo
// actualiza los campos provistos (no nulos), para no pisar datos buenos.
export function setMeta(id, { title = null, durationMs = null, artist = null } = {}) {
  const sets = [], vals = []
  if (title != null && title !== '') { sets.push('title = ?'); vals.push(title) }
  if (durationMs != null && !isNaN(durationMs)) { sets.push('duration_ms = ?'); vals.push(Math.round(durationMs)) }
  if (artist != null && artist !== '') { sets.push('artist = ?'); vals.push(artist) }
  if (!sets.length) return getById(id)
  vals.push(Number(id))
  getDb().prepare(`UPDATE songs SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  return getById(id)
}
