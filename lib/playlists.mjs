...
export function listAllWithOwner() {
  ...
  // Carátula tipo YouTube Music: ids de las primeras 4 canciones con arte.
  const coverStmt = db.prepare(
    `SELECT s.id FROM playlist_items pi JOIN songs s ON s.id = pi.song_id
     WHERE pi.playlist_id = ? AND s.art_key IS NOT NULL
