...
export const config = {
  ...
  musicCacheMaxBytes: Number(process.env.MUSIC_CACHE_MAX_BYTES || 20 * 1024 ** 3), // 20 GB (caché LOCAL de música, solo la usa el fallback geo-restringido)
  s3: {
    ...
    // Base pública para servir objetos por CDN/dominio (R2 no sirve por el endpoint S3).
    // Ej: https://pub-xxxx.r2.dev  o  https://cdn.midominio.com  (ya apunta al bucket).
